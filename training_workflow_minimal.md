...
现在训练模型可以正确开启并完成，但是训练效果仍旧存在问题。
1. 设置lr是>1e-4，训练集上loss也没有明显下降，处于振荡。
2. 设置lr<1e-4，比如5e-5，训练集上loss下降较为平衡，rank_ic提升也较为平滑。但是验证集没有出现明显的loss下降和ic上升，然后就由于ic不降导致早停了（走了33个epoch，设置的100）。batch设置的4。

我认为主要原因有两个。
1. 是卷积层股票*时序因子结合批归一化，导致各类因子和股票的对应的时序信息分布混淆。
2. 是因子注意力和[CLS]token结合的信息降维中导致了因子混淆（例如高价低量和高量低价在[CLS]token中是无法被分辨的。）

修改思路：
1. 不对[股票，因子]维度做展平，而是把股票看作批次，因子看作通道。卷积后的通道相当于隐时序因子。卷积模块中不再使用批归一化，而是使用GroupNorm，组数可以设置32。 1D 卷积通道数先升维到512，再最终降维到256。
2. 卷积模块结束后，现在是把几个通道的T_compressed拼接作为token编码，然后进行因子注意力。这里由于通道作为隐因子了，直接将T_compressed作为token编码即可。标量因子升维需要对齐到对应的token编码长度。然后需要增加位置编码，用于解决[CLS]token中模式混淆的问题。
3. 后续股票维度的注意力仍旧保持无位置编码。